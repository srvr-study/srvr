...
public class AuthController {
    ...
    public ApiResponse<String> requestSendCodeMail(@PathVariable String email) {
        ...
        if (isSuccess) {
            ...
            return new FailureResponse<>(ErrorCode.INVALID_REQUEST, "메일 서버 문제로 메일 전송에 실패했습니다. 잠시 후 다시 시도해 주세요.");
        }
    }

    @PostMapping("/{email}/code-check")
    public ApiResponse<Boolean> checkFindPasswordCode(@PathVariable String email) {
        // todo 코드 DTO
        log.info("[auth] checkFindPasswordCode {}, {}",email);
        return null;
    }
}
